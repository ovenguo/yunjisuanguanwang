...
<link href="__PUBLIC__/css/bootstrap.css" rel="stylesheet" type="text/css" media="all" /> 
 ...
 while($result=mysql_fetch_array($notice)){ ?>
        <a target="_top" href='<?php echo U('Index/Index/noticearticle',array('id'=>$result['id'],'type'=>$type));?>'><li><strong><?php echo $result['title'];?></strong><span style="float:right"><?php echo "发布时间：".date('Y-m-d H:i',$result['date']) . " 编辑：".$result['username'];?></span></li></a>
        <?php }?>
      </ul>
